...
class DigitsModel(nn.Module):
  ...
  def __init__(self):
    ...
    self.embed = nn.Sequential(
        nn.Conv2d(1, 16, kernel_size=3, stride=(
            2, 1), padding=(1, 1), bias=False),
        nn.ELU(),
        nn.Conv2d(16, 32, kernel_size=3, stride=(
            1, 1), padding=(1, 1), bias=False),
        nn.ELU(),
        nn.Conv2d(32, 64, kernel_size=3, stride=(
            2, 2), padding=(1, 1), bias=False),
        nn.ELU(),
        nn.Conv2d(64, 32, kernel_size=3, stride=(
            1, 1), padding=(1, 1), bias=False),
        nn.ELU()
    )
...
def run_eval(model, test_dataset):
  error_rate_op = ER()
  loader = DataLoader(test_dataset, num_workers=5,
